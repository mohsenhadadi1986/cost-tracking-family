...
describe('OpenAPI and Swagger integration', () => {
  describe('GET /api/openapi.json', () => {
    it('returns 200 with a valid OpenAPI spec', async () => {
      ...
      assert.ok(spec.paths['/api/accounts'].get);
      ...
      assert.ok(spec.paths['/api/tax/730/years']);
      assert.ok(spec.paths['/api/tax/730/{year}']);
      assert.ok(spec.paths['/api/tax/730/{year}/cu']);
    });
  });
